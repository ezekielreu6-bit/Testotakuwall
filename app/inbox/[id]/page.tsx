...
export default function ChatRoom() {
  ...
  useEffect(() => {
    ...
    const unsubChat = onSnapshot(doc(db, "chats", id), async (docSnap) => {
      if (docSnap.exists()) {
        ...
        // Pre-fetch profile data for Details panel
  ...
  const toggleBlock = async () => {
    ...
    await updateDoc(doc(db, "users", user.uid), {
      blockedUids: isBlocked ? arrayRemove(otherUid) : arrayUnion(otherUid)
    });
  };
  ...
  return (
    <main className="flex flex-col h-screen bg-black text-white relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 min-w-0" onClick={() => setShowInfo(true)}>
            ...
            <div className="min-w-0">
              <h3 className="text-[16px] font-black truncate leading-tight flex items-center gap-1">
                {chatData.chatName}
                {isBlocked && <Slash className="w-3 h-3 text-red-500" />}
              </h3>
              <div className="flex items-center gap-1.5">
                {chatData.syncCount && chatData.syncCount >= 3 ? (
                    ...
                    <div className="flex items-center gap-1">
                      <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${isBlocked ? 'bg-zinc-600' : 'bg-green-500'}`} />
                      <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest">{isBlocked ? 'Blocked' : 'Active Sync'}</span>
                    </div>
                )}
              </div>
            </div>
      ...
      {/* 🔵 MESSAGES AREA: Grouped Sender Spacing */}
      <div className="flex-1 overflow-y-auto p-4 flex flex-col no-scrollbar pt-[125px] pb-32">
        {messages.map((m, i) => {
          const isMe = m.senderId === user.uid;
          const isSameSender = i > 0 && messages[i - 1].senderId === m.senderId;
          
          return (
            <div key={m.id} className={`flex w-full ${isMe ? 'justify-end' : 'justify-start'} ${isSameSender ? 'mt-1' : 'mt-5'}`}>
              <div 
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-zinc-950/95 backdrop-blur-2xl border-t border-white/5 p-4 pb-safe z-50">
        {isBlocked ? (
            <div className="py-2 flex flex-col items-center gap-1 opacity-50">
                <ShieldAlert className="w-4 h-4 text-red-500" />
                <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400">You have blocked this sync</p>
            </div>
        ) : (
            <div className="flex items-center gap-3 max-w-4xl mx-auto">
                ...
                <button 
                  ...
                  disabled={!input.trim()}
                  className="bg-red-600 p-2.5 rounded-full text-white active:scale-90 transition disabled:opacity-40"
                >
      ...
      <div className={`sub-view fixed inset-0 bg-black z-[200] transition-transform duration-500 flex flex-col ${showInfo ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 no-scrollbar pb-10">
          <div className="flex flex-col items-center mb-10 text-center">
            ...
            <p className="text-[10px] text-zinc-500 font-bold uppercase mt-1 tracking-[2px]">{chatData.isGroup ? "Sync Group" : "Private Sync"}</p>
          </div>

          <div className="space-y-2">
             <ActionBtn 
               icon={<MapPin className={`w-5 h-5 ${isPinned ? 'text-red-500 fill-red-500' : 'text-zinc-500'}`} />} 
               label={isPinned ? "Unpin Chat" : "Pin to Top"} 
               onClick={togglePin} 
             />
             {!chatData.isGroup && (
               <ActionBtn 
                 icon={<Slash className={`w-5 h-5 ${isBlocked ? 'text-green-500' : 'text-red-500'}`} />} 
                 label={isBlocked ? "Unblock User" : "Block User"} 
                 onClick={toggleBlock} 
               />
             )}
             <ActionBtn icon={<Trash2 className="text-red-600 w-5 h-5"/>} label={chatData.isGroup ? "Leave Group" : "Delete Chat"} />
          </div>
              ...
              {participants.map(p => (
                <div key={p.uid} className="flex items-center justify-between bg-zinc-900/50 p-4 rounded-3xl border border-white/5" onClick={() => router.push(`/user/${p.uid}`)}>
                   <div className="flex items-center gap-3">
                      <img src={p.photoURL} className="w-10 h-10 rounded-2xl object-cover" alt="p" />
      ...
      {contextMsg && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/60 backdrop-blur-md px-6" onClick={() => setContextMsg(null)}>
          <div className="w-full max-w-xs bg-zinc-900 rounded-[35px] overflow-hidden border border-white/10 shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
             ...
             <ContextRow icon={<Copy className="w-5 h-5 text-zinc-400"/>} label="Copy Text" onClick={() => { navigator.clipboard.writeText(contextMsg.text || ""); setContextMsg(null); }} />
             {contextMsg.senderId === user.uid && (
               <ContextRow icon={<Trash2 className="w-5 h-5 text-red-500"/>} label="Delete Message" onClick={() => { deleteDoc(doc(db, `chats/${id}/messages`, contextMsg.id)); setContextMsg(null); }} />
             )}
          </div>
        </div>
      )}
...
function ContextRow({ icon, label, onClick }: any) {
    return (
      <button onClick={onClick} className="w-full p-5 flex items-center gap-4 text-sm font-bold border-b border-white/5 last:border-0 active:bg-white/5 transition">
...
function LinkPreview({ text, isMe }: { text: string, isMe: boolean }) {
  ...
  return (
    <Link href={`/watch/${videoId}`} className={`mt-3 block overflow-hidden rounded-2xl border ${isMe ? 'border-white/20' : 'border-white/10'} bg-black group`}>
      ...
      <div className="p-2.5 bg-zinc-900/90 text-[11px] font-black truncate">{video.title}</div>
    </Link>
  );
}