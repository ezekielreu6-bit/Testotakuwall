...
export default function ChatRoom() {
  ...
  return (
    <main className="flex flex-col h-screen bg-black text-white relative overflow-hidden">
      ...
      {/* 🟢 FIXED HEADER: Responsive with notch support */}
      ...
      {/* 🔵 MESSAGES AREA: Large gaps for reactions */}
      <div className="flex-1 overflow-y-auto p-4 flex flex-col no-scrollbar pt-[110px] pb-40">
        {messages.map((m, i) => {
          const isMe = m.senderId === user.uid;
          const isSameSender = i > 0 && messages[i - 1].senderId === m.senderId;

          return (
            <div key={m.id} className={`flex w-full ${isMe ? 'justify-end' : 'justify-start'} ${isSameSender ? 'mt-3' : 'mt-8'}`}>
              <div 
              ...
              >
                {(m.type as string) === 'sticker' ? (
                  ...
                  <div className={`
                    px-4 py-2.5 rounded-[22px] text-[15px] font-medium leading-normal w-fit shadow-lg
                ...
                {/* 🏷 REACTION PILL: Absolute bottom with space */}
                {m.reaction && (
                  <div className={`absolute -bottom-4 ${isMe ? 'right-1' : 'left-1'} bg-zinc-900 border border-white/10 rounded-full px-2 py-0.5 text-[12px] shadow-2xl z-10 flex items-center justify-center`}>
                    {m.reaction}
                  </div>
                )}
              </div>
      ...
      {/* 🟠 CONTEXT MENU: Higher Z-index */}
      {contextMsg && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-black/60 backdrop-blur-md px-6" onClick={() => setContextMsg(null)}>
      ...
      {/* 🔴 SYNC INFO DRAWER: Highest possible Z-Index to stay on top */}
      <div className={`fixed inset-0 z-[400] transition-transform duration-500 flex flex-col bg-black ${showInfo ? 'translate-x-0' : 'translate-x-full'}`}>
        <header className="p-6 border-b border-white/5 flex items-center gap-4 pt-safe bg-zinc-950">
          <button onClick={() => setShowInfo(false)} className="p-2 bg-zinc-900 rounded-full active:scale-90 transition shadow-lg"><ArrowLeft className="w-5 h-5"/></button>
        ...
        <div className="flex-1 overflow-y-auto p-6 pb-10">
          <div className="flex flex-col items-center mb-10 text-center">
            ...
            <p className="text-[10px] text-zinc-500 font-bold uppercase mt-1 tracking-[2px]">{chatData.isGroup ? "Direct Group Session" : "Private Sync Session"}</p>
          </div>
          ...
          <div className="mt-10">
            ...
            <div className="space-y-3">
              {participants.map(p => (
                <div key={p.uid} className="flex items-center justify-between bg-zinc-900/50 p-4 rounded-3xl border border-white/5" onClick={() => { setShowInfo(false); router.push(`/user/${p.uid}`); }}>
